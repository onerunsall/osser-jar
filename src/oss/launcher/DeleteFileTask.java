	...
	public void run() {
		...
		try {
			sql = new StringBuilder(
					"select path,addTime,delay from t_todel where addTime=null or delay=0 or (addTime is not null and delay>0 and timestampdiff(SECOND,addTime,now())>delay) order by delay asc,addTime desc limit 0,50");
			sql1 = new StringBuilder("select id fildId,cover,path,tmpIf from t_file where id=? ");
			sql3 = new StringBuilder("delete from t_todel where path=?");
			sql2 = new StringBuilder("delete from t_file where id=?");
			sql4 = new StringBuilder("insert into t_todel (path) values(?)");
			sql5 = new StringBuilder(
					"insert into t_todel(path) select a.path  from t_file a where a.tmpIf=1 and now() >SUBDATE(a.addTime,interval -3 day)");
